...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:user="urn:user-namespace-here" version="1.0">
  ...
  <xsl:template match="htPage">
	...
	<body>
	  ...
	  <form id="Form1" method="POST" name="Form1"  action="UpdateAction.do" onSubmit="return checkOnSubmit()" >
		<table cellspacing="0">
		...
		<input type="hidden" name="pk">
			<xsl:attribute name="value"><xsl:value-of select="htForm/formModel/fieldList/field[isPrimaryKey = 'Y']/fieldName" /></xsl:attribute>
		</input>
            	<input type="submit" value="新增存檔" class="cbutton" />
            	<input type="button" value="刪除" class="cbutton" >
            		<xsl:attribute name="onclick">
            		if( confirm('確定刪除?') ){document.Form1.action = 'DeleteAction.do';document.Form1.submit();}
            		</xsl:attribute>            		
            	</input>
