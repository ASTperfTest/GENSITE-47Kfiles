...
if request("submitTask")="LIST" then
	fSql = "SELECT htx.iCUItem,htx.sTitle,htx.Created_Date,C9.giCuItem,C9.MMOFileName,C9.MMOFolderName,C9.MMOFileIcon,htx.iEditor,htx.dEditDate,xref1.mValue " & _
		...
		"LEFT JOIN CodeMain AS xref1 ON xref1.mCode = C9.MMOFileType AND xref1.codeMetaID=N'fileType' " & _
		"WHERE C9.MMOFileName is not null "
	if request("htx_xKeyword")<>"" then
		fSql=fSql&" AND xKeyword Like N'%"+request("htx_xKeyword")+"%'"
		xKeyword=request("htx_xKeyword")
	end if
	if request("htx_MMOFileName")<>"" then
		fSql=fSql&" AND MMOFileName Like N'%"+request("htx_MMOFileName")+"%'"
		xMMOFileName=request("htx_MMOFileName")
	end if
	if request("htx_MMOFolderName")<>"" then
		fSql=fSql&" AND MMOFolderName = N'"+request("htx_MMOFolderName")+"'"
		xMMOFolderName=request("htx_MMOFolderName")
	end if	
	if request("htx_fileType")<>"" then
		fSql=fSql&" AND MMOFileType Like N'"+request("htx_fileType")+"%'"
	...
	if request("htx_IDateS") <> "" then
		...
		fSql = fSql & " AND htx.Created_Date BETWEEN N'"+rangeS+"' and N'"+rangeE+"'"
	end if
'response.write fSql
'response.end	
...
<option value="">請選擇</option>
			<%SQL="Select mCode,mValue from codeMain where codeMetaID=N'fileType' Order by mSortValue"
...
<option value="">全部</option>
			<%SQL="Select MMOFolderID,MMOFolderName,MMOFolderDesc from MMOFolder order by MMOFolderName"
			SET RSS=conn.execute(SQL)
			While not RSS.EOF%>

			<option value="<%=RSS(1)%>"><%=RSS(1)%>(<%=RSS(2)%>)</option>
			<%	RSS.movenext
			wend%>
</select>
            </td>
     	...
     	<td class=center height=100 width=60 onclick="editCall '<%=RSreg("iCUItem")%>','<%=RSreg("MMOFolderName")%>'">
     	  ...
     	  <%if not isNull(RSreg("MMOFileName")) then%>
     	    ...
     	    <%if not isnull(RSreg("MMOFileIcon")) then%>
     	    	<img id="logo_MMOFileName" src="<%=session("mySiteURL")%><%=session("MMOPath")%><%=RSreg("MMOFolderName")%>/<%=RSreg("MMOFileIcon")%>">
     	    <%else
     	    	xFileExt="":xIcon=""
...
%>
  <tr>
    ...
    <td class="Center"><%=RSreg("Created_Date")%></td>
  </tr>
<%	     	RSreg.movenext
	    wend
    	end if
...
<script language=vbs>
sub AddMMO()
	window.navigate "CuMMOAdd.asp","","scrollbars=yes,width=700,height=500"
end sub
...
sub closeForm()
  	...
  	if radiochk then 
		...
		window.opener.doInsertImage MMOPath,xTitle
		window.close
	else
		alert "請選取物件!"
		exit sub
...
sub clientInitForm
    <%if request("submitTask")="LIST" then%>
    	...
    	PopForm.htx_MMOFolderName.value="<%=xMMOFolderName%>"
