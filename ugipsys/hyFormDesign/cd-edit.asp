...
'----------HyWeb GIP DB CONNECTION PATCH----------
	...
	while not RS.eof
%>
<SPAN id=d-<%=RS("part_id")%> title="<%=RS("posHeight")%>" style="align:center; border-width:2; border-style:outset; border-color:#999999; background-color:<%=RS("bgcolor")%>;
...
'		msgBox xSpanArray(xSortArray(xTopI,1),1)
		cxID = mid(xSpanArray(xSortArray(xTopI,1),1),3)
		document.all("d-"&cxID).style.top = rxTop
		document.all("d-"&cxID).style.width = c1Width
		document.all("d-"&cxID).title = xSpanArray(xSortArray(xTopI,1),4)
		document.all("vbt"&cxID).style.top = rxTop + document.all("d-"&cxID).style.posHeight - 10
...
'		mObj.style.borderStyle = "groove"
		mObj.style.zIndex = 1
		window.status = "調整寬度"
	elseif (xmid = "") and (pObj.tagName = "SPAN") then 
		set mObj = pObj
		xtype = "SPAN"
		xmid = mObj.id
		dxmid = mObj.id
...
'  	msgBox xObj.style.left
	...
	elseif left(xmid,3)="vbt" then
		set spxObj=document.all("d-"&mid(xmid,4))
