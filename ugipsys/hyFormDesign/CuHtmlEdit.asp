...
'	  	htPageDom.selectSingleNode("//pHTML").replaceChild htPageDom.createCDATASection(xBody), htPageDom.selectSingleNode("//pHTML").childNodes.Item(0)
		...
		writeCodeStr = ""
		for each param in htPageDom.selectNodes("//dsTable[tableName='formUser']/fieldList/field")
			response.write nullText(param.selectSingleNode("fieldName")) & "<HR>"
			processValid param
		next
...
'		response.end
		...
		if isNumeric(iCuItem) then
			sql = "UPDATE CuDtxDFormSpec SET xmlSpec = " & pkStr(htPageDom.xml,"") _
				& ", pHTML = " & pkStr(xBody,"") _
				& ", formScript = " & pkStr(writeCodeStr,"") _
