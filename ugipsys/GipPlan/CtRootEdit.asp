...
'  End if
  ...
  reg.Submit
   ...
   sub formDelSubmit()
         chky=msgbox("注意！"& vbcrlf & vbcrlf &"　你確定刪除此目錄樹與其下所有節點嗎？"& vbcrlf , 48+1, "請注意！！")
        if chky=vbok then
  ...
  sub copyTree_onClick	
  	window.navigate "CtRootCopy.asp?<%=pKey%>&orgTree=" & document.all("orgTree").value
  end sub

</script>

<!--#include file="CtRootFormE.inc"-->
...
<div id="FuncName">
	<h1><%=Title%></h1>
	<div id="Nav">
 <% if (HTProgRight and 1)=1 then %><a href="CtNodeTList.asp?<%=pKey%>">編修目錄樹</a><!--<a href="CtRootQuery.asp">編修目錄樹</a>--><% End IF %>
 <% if (HTProgRight and 1)=1 then %><a href="CtRootDataList.asp?<%=pKey%>">上稿統計</a><!--<a href="CtRootQuery.asp">編修目錄樹</a>--><% End IF %>
 <!--<% if (HTProgRight and 2)=2 then %><a href="CtRootQuery.asp">查詢</a><% End IF %>-->
