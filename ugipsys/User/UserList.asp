    ...
    <td class="FormLink" valign="top" width="40%">
      <p align="right">
      	<%if (HTProgRight and 4)=4 then%>      
      		<a href="UserAdd.asp">新增使用者</a>&nbsp;
        <%end if%>
      	<%if (HTProgRight and 2)=2 then%>      
      		<a href="UserQuery.asp">重設查詢</a>&nbsp;
        <%end if%>
       	 <span id=RunJob style="visibility:hidden">
   	   <input type=button class=cbutton value="修改權限" id=button1>
	   </span>	
      	</td>      		
  </tr>
  <tr>
...
<%                   
	...
	userLoginId = ""
	Dim idnLength 
	idnLength = Len(trim(RSreg("UserID")))
	if idnLength >9 then
		for iii = 0 to (idnLength-5)
			userLoginId = userLoginId & "*"
		next
		userLoginId = userLoginId & Right(RSreg("UserID"),4)
	else
		userLoginId = trim(RSreg("UserID"))
	end if
%>                 
<tr>   
	...
	<TD class="whitetablebg"><a href="../User/UserEdit.asp?UserID=<%=RSreg("UserID")%>"><%=userLoginId%></a></TD>   
