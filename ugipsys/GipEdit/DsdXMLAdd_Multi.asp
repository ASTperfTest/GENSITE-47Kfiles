...
<%
    SQLCheck = "Select rdsdcat from BaseDsd B Left Join CuDtGeneric C ON B.ibaseDsd = C.ibaseDsd where icuitem=" & pkStr(request.querystring("icuitem"),"")
    Set RSCheck = conn.execute(SQLCheck)
    if not isNull(RSCheck(0)) then xrdsdcat = RSCheck(0)
    userId = session("userId")
