...
HTProgCode="GC1AP1"
HTProgPrefix="DsdXML" 
' ============= Modified by Chris, 2006/08/23, to handle "multiUpload" InputType ========================'
'		Document: 950822_智庫GIP擴充.doc
'  modified list:
'	Sub AddFileUpload(xField)
'	Sub DelFileUpload(xField, tableno)
%>
...
sub popCalendar(dateName,followName)        
 ...
 '      ey=document.body.scrolltop+window.event.clientY+10
       ey = window.event.srcElement.parentElement.offsetTop + 10
...
<%
	for each xparam in allModel2.selectNodes("//fieldList/field[formList!='']") 
	    	if not (nullText(xparam.selectSingleNode("fieldName"))="fctupublic" and session("CheckYN")<>"N") then
...
' ===begin========== Modified by Chris, 2006/08/23, to handle "multiUpload" InputType ========================
Sub AddFileUpload(xField)
	NowFileInputCount = cint(document.all("AddUploadCount_" & xField).value)
	NowFileInputNo = cint(document.all("AddUploadMax_" & xField).value)
  If NowFileInputCount < 10 Then
   HTMLSrc = "<table border=0 cellspacing=0 cellpadding=0 Id=OutloadTable_" & xField &"_"& NowFileInputNo &">"& vbcrlf
   HTMLSrc = HTMLSrc & "<tr><td>" & vbcrlf
   HTMLSrc = HTMLSrc & "<input type=file name=UploadFile_"& xField &"_"& NowFileInputNo _
		&" size=40><input type=button class=cbutton value=取消 OnClick=""VBS: DelFileUpload '" & xField & "'," & NowFileInputNo &""">" & vbcrlf
   HTMLSrc = HTMLSrc & "</td></tr></table>"
   document.all("AddUpload_"&xField).InsertAdjacentHTML "beforeBegin", HTMLSrc
   document.all("AddUploadMax_" & xField).value = NowFileInputNo + 1
   document.all("AddUploadCount_" & xField).value = NowFileInputCount + 1
  Else
   Alert "太多附件了吧..."
  End IF
 End Sub
 ...
 Sub DelFileUpload(xField, tableno)
  	document.all("OutloadTable_" & xField &"_"& tableno).outerHTML = ""
     document.all("AddUploadCount_" & xField).value =  cint(document.all("AddUploadCount_" & xField).value) - 1
 End Sub
' ===end========== Modified by Chris, 2006/08/23, to handle "multiUpload" InputType ========================
...
<div id="FormName">
	       ...
	       <input type="button" value="一般資料式" onclick="window.location.href='<%=HTprogPrefix%>Add.asp?phase=add'"></input>
	       <input type="button" value="URL連結式" onclick="window.location.href='<%=HTprogPrefix%>Add.asp?showType=2&phase=add'"></input>
	       <input type="button" value="檔案下載式" onclick="window.location.href='<%=HTprogPrefix%>Add.asp?showType=3&phase=add'"></input>	       
...
sub doUpdateDB()
	...
	if checkGIPconfig("onlyThisNode") then
		sql = sql & "iNode,"
		sqlValue = sqlValue & session("ctNodeId") & ","
	end if
	...
	for each param in allModel.selectNodes("dsTable[tableName='CuDtGeneric']/fieldList/field[formList!='']") 
		...
		elseif not ((nullText(param.selectSingleNode("fieldName"))="fctupublic" and session("CheckYN")<>"N") or (nullText(param.selectSingleNode("identity"))="Y")) then
			processInsert param
		end if
	next
...
'-------- 950703 Chris 擴充萬用外部附加處理 (extraServiceURI) -----begin------------------------
	for each param in allModel.selectNodes("extraServiceURI")
		xURI = session("mySysSiteURL") & "/wsxd/" & nullText(param) & "?iCuItem=" & xNewIdentity
'		response.write "Calling " & xURI & "<HR>"
	...
	next
	for each param in allModel.selectNodes("extraServerExecute")
		xURI = "/site/" & session("mySiteID") & "/wsxd/" & nullText(param)
'		response.write "Calling " & xURI & "<HR>"
		server.execute xURI
	next
'	response.end
...
<%		
	elseif session("replyTitle") <> "" and session("replyID") <> 0 then
		nextURL = HTprogPrefix & "View.asp?icuitem=" & session("replyID") & "&phase=edit"
