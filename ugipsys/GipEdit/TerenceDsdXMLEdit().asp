...
'  modified list:
'	『新增』連結加 nullText(htPageDom.selectSingleNode("//sBaseDSDXML/addYN"))<>"N" 條件
' ============= Modified by Chris, 2006/08/23, to handle "multiUpload" InputType ========================'
'		Document: 950822_智庫GIP擴充.doc
'  modified list:
'	Sub AddFileUpload(xField)
'	Sub DelFileUpload(xField, tableno)
' 	sub initMultiUpload(xField, ixCuUpload, apath, oFileName, nFileName)
'--- 發現重覆 initial, window_onLoad 裡已 clientInitForm 過了
'	mark掉 script 最後的 clientInitForm 敍述
%>
...
sub initMultiUpload(xField, ixCuUpload, apath, oFileName, nFileName)
	HTMLSrc = "<INPUT type=checkbox name=DelMultiUpload_" & xField & " value=" & ixCuUpload & " />刪除　" _
		& "<A href=""<%=HTUploadPath%>" & apath & "/" & nFileName & """>" & oFileName & "</A><BR/>"
	document.all("AddUpload_"&xField).InsertAdjacentHTML "beforeBegin", HTMLSrc
end sub
