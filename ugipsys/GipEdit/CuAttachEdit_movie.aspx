  ...
  ' 檢查檔案是否有內容
   ...
   If file.ContentLength <>0 Then
    ...
    file2.Delete(up_path & "/" & request("htx_nfilename"))
     ...
     objCmd.CommandText = "update CuDTAttach set aTitle=N'" & request("htx_aTitle") & "',aDesc=N'" & request("htx_aDesc")  & "',NFileName=N'" & nfname & "',aEditor=N'" & htx_user & "',aEditDate=getdate(),AttachkindA=N'" & request("htx_AttachkindA") & "',Attachtype=N'" & request("htx_Attachtype") & "',bList=N'" & request("htx_bList") & "',listSeq=N'" & request("htx_listSeq") & "' where ixCuAttach=N'" & htx_ixCuAttach &"'" 
     objCmd.ExecuteNonQuery()
...
<%
     ...
     objCon.Open() ' 開啟資料庫連結
     objCmd = New SqlCommand(strSQL, objCon)
     objCmd.CommandText = "SELECT htx.xicuitem,htx.atitle,isnull(htx.adesc,'') adesc,htx.nfilename,htx.aeditor,htx.aeditdate,htx.AttachkindA,htx.Attachtype,htx.blist,isnull(htx.listseq,'') listseq, isnull(xrefNFileName.oldFileName,'')  AS fxr_NFileName FROM (CuDTAttach AS htx LEFT JOIN imageFile AS xrefNFileName ON xrefNFileName.newFileName = htx.NFileName) WHERE htx.ixCuAttach=" & request("ixCuAttach")
    ...
    while objDataReader.Read() 
      ...
      AttachkindA=trim(objDataReader.Item("AttachkindA"))
      Attachtype=trim(objDataReader.Item("Attachtype"))
...
<Select name="htx_AttachkindA">
<% if AttachkindA="1" then  %>
<option value="">請選擇</option>
<option value="1" selected>影音─WMV／AVI／MPG</option>
<option value="2">影音─MOV</option>
<option value="3">圖檔</option>
<option value="4">文字</option>
<option value="5">Flash檔</option>
<% end if %>
<% if AttachkindA="2" then  %>
<option value="">請選擇</option>
<option value="1">影音─WMV／AVI／MPG</option>
<option value="2" selected>影音─MOV</option>
<option value="3">圖檔</option>
<option value="4">文字</option>
<option value="5">Flash檔</option>
<% end if %>
<% if AttachkindA="3" then  %>
<option value="">請選擇</option>
<option value="1">影音─WMV／AVI／MPG</option>
<option value="2" selected>影音─MOV</option>
<option value="3" selected>圖檔</option>
<option value="4">文字</option>
<option value="5">Flash檔</option>
<% end if %>
<% if AttachkindA="4" then  %>
<option value="">請選擇</option>
<option value="1">影音─WMV／AVI／MPG</option>
<option value="2" selected>影音─MOV</option>
<option value="3">圖檔</option>
<option value="4" selected>文字</option>
<option value="5">Flash檔</option>
<% end if %>
<% if AttachkindA="5" then  %>
<option value="">請選擇</option>
<option value="1">影音─WMV／AVI／MPG</option>
<option value="2" selected>影音─MOV</option>
<option value="3">圖檔</option>
<option value="4">文字</option>
<option value="5" selected>Flash檔</option>
<% end if %>
<% if AttachkindA="" then  %>
...
<TD class="eTableContent">
<Select name="htx_Attachtype">
<% if Attachtype="1" then  %>
<option value="">請選擇</option>
<option value="1" selected>寬頻</option>
<option value="2">窄頻</option>
<% end if %>
<% if Attachtype="2" then  %>
<option value="">請選擇</option>
<option value="1">寬頻</option>
<option value="2" selected>窄頻</option>
<% end if %>
<% if Attachtype="" then  %>
<option value="" selected>請選擇</option>
<option value="1">寬頻</option>
