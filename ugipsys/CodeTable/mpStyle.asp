...
<%
		SQL="Select mCode,mValue from codeMain where mSortValue IS NOT NULL AND codeMetaID=N'xStyle' Order by mSortValue"
			...
			While not RSS.EOF%>
			<INPUT TYPE="radio" NAME="MpStyle" value="<%=RSS(0)%>">
			<IMG SRC="<%=RSS(0)%>.gif" width="150" align="top"><%=RSS(1)%>
			<BR/>
			<%	RSS.movenext
			wend%>
...
<INPUT TYPE=hidden name=CalendarTarget>
   ...
   <input type="button" class=cbutton value="清除重填" onClick="resetForm()">
</form>     
