...
<form id="reg" method="POST" name="reg" action="" ENCTYPE="MULTIPART/FORM-DATA">
	...
	<TR>
		...
		<TD class="eTableContent"><!--input name="xStatus1" size="2" title="" value="<%=rs("xStatus")%>" readonly="true" class="rdonly"-->
			<Select name="xStatus">
			<option value="Y" <%if rs("xStatus") = "Y" then response.write "selected" end if%>>正常</option>
			<option value="D" <%if rs("xStatus") = "D" then response.write "selected" end if%>>刪除</option>
			</select>
		</TD>
	</TR>
	<TR>
