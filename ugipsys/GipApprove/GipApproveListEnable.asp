...
<%	   
    ...
    if nowpage="" then
    	fSql="Select C.icuitem,C.stitle,C.xkeyword,CT.ctUnitName,CM.mvalue xreftopDataCat,D.deptName,C.deditDate,CTN.catName,C.fctupublic,CM2.mvalue xreffctupublic,CTR.pvXdmp " & _
	...
	if request("ctNodeId")<>"" then
		...
		fSql=fSql&" AND C.fctupublic=N'P'"
	end if
...
<script Language="javascript">
    function enableTest(myUrl)
    {
        var newURL;
        newURL ="http://enable.nat.gov.tw/Servlet1?inputURL="+ encodeURIComponent(myUrl) +"&analyzingGrade=APlus"
        //alert(newURL);
 ...
 <INPUT TYPE=hidden name=fctupublic value=""> 
  ...
  <tr>
    <td class="FormLink" valign="top" align=right colspan="2">
    	 是否公開: <Select name="sfx_fctupublic" size=1>
...
If not RSreg.eof then   
    ...
    for i=1 to PerPageSize
        ...
        'enableURL =Replace(Replace(Replace(Replace(Replace(xUrl,":","%3A"),"/","%2F"),"?","%3F"),"=","%3D"),"&","%26")
        'enableURL = encodeURI(xUrl)
        'enableURL = ="http://enable.nat.gov.tw/Servlet1?inputURL="& xUrl &"&analyzingGrade=APlus"
%>                  
<tr>                  
...
<INPUT TYPE=hidden name="xphKeyicuitem<%=i%>" value="<%=RSreg("icuitem")%>"></td>   
	...
	<TD class=eTableContent><font size=2>
	<%=RSreg("deditDate")%></TD>
	<td align=center class=eTableContent><A href="#" onclick="enableTest('<%=xUrl%>')">檢測</A>&nbsp;</td>
