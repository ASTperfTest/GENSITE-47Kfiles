...
elseif request("submitTask") = "edit" then
	...
	sql = "UPDATE CuDTGeneric SET dEditDate=getdate(), xBody = " & xBody & ", xURL = " & xURL & ", xNewWindow = " & xNewWindow & ", " & _
				"fCTUPublic = " & fCTUPublic & ", vGroup = " & vGroup & ", xKeyword = " & xKeyword & ", xImportant = " & xImportant & ", " & _
				"iDept = " & iDept & " WHERE iCUItem = " & discussId
	conn.execute(sql)
	'刪除救回
	sql2_1 = "SELECT * FROM KnowledgeForum WHERE gicuitem = " & discussId
	set rs2_1 = conn.execute(sql2_1)
	if request("KStatus") = "N" and rs2_1("Status") <> request("KStatus") then
		sql3 = "UPDATE KnowledgeForum SET DiscussCount = DiscussCount +1 , "
		sql3 = sql3 & " CommandCount = CommandCount + "&rs2_1("CommandCount") &","
		sql3 = sql3 & " GradeCount = GradeCount + "&rs2_1("GradeCount") &","
		sql3 = sql3 & " GradePersonCount = GradePersonCount + "&rs2_1("GradePersonCount")
		sql3 = sql3 & " WHERE gicuitem = " & questionId
		conn.execute(sql3)
	elseif request("KStatus") = "D" and rs2_1("Status") <> request("KStatus") then
		sql3 = "UPDATE KnowledgeForum SET DiscussCount = DiscussCount -1 , "
		sql3 = sql3 & " CommandCount = CommandCount - "&rs2_1("CommandCount") &","
		sql3 = sql3 & " GradeCount = GradeCount - "&rs2_1("GradeCount") &","
		sql3 = sql3 & " GradePersonCount = GradePersonCount - "&rs2_1("GradePersonCount")
	...
	sql2 = "UPDATE KnowledgeForum SET Status = "& KStatus &" WHERE gicuitem = " & discussId
	conn.execute(sql2)
	'刪除救回end

	'2010.4.29 Add by Ivy 活動新增 討論不公開或刪除,扣除活動積分
	if request("fCTUPublic") = "N" or request("KStatus")="D" then
		ActivityDiscuss discussId ,"false"
		sql4 = "UPDATE CuDTGeneric SET Abstract = 'Y',dEditDate=getdate() WHERE iCUItem = " & discussId
		conn.execute(sql4)
	else
		ActivityDiscuss discussId ,"true"
		sql4 = "UPDATE CuDTGeneric SET Abstract = 'N',dEditDate=getdate() WHERE iCUItem = " & discussId
		conn.execute(sql4)
	end if
...
<% Sub showDoneBox(lMsg) %>
  <html>
    ...
    <body>
			<script language=vbs>
			  alert("<%=lMsg%>")	    				
				document.location.href="<%=HTprogPrefix%>List.asp?questionId=<%=questionId%>"
			</script>
    </body>
  </html>
<% End sub %>
...
<div id="FuncName">
    ...
    <a href="knowledgepiclist.asp?iCUItem=<%=discussId%>&nowPage=<%=request.querystring("nowPage")%>&pagesize=<%=request.querystring("pagesize")%>">圖片管理</a>&nbsp;
    <!-- a href="specialistlist.htm">專家補充管理</a>&nbsp; -->
    <a href="/KnowledgeForum/KnowledgeDiscussList.asp?questionId=<%=questionId%>&nowPage=<%=request.querystring("nowPage")%>&pagesize=<%=request.querystring("pagesize")%>">討論管理</a>&nbsp;
    <!-- a href="publishset.htm">發佈至主題館</a>&nbsp; -->
...
<div id="FormName">
	單元資料維護&nbsp;
  <font size=2>【編輯(<font color=red>一般資料式</font>)--主題單元:(知識家)-知識討論 / 單元資料:知識家討論區】
</div>

	<form id="Form1" method="POST" name="reg" action="KnowledgeDiscussContent.asp?questionId=<%=questionId%>&discussId=<%=discussId%>" >
		...
		<TR>
			...
			<TD class="eTableContent">
				<input name="iEditor" class="rdonly" value="<%=trim(rs("iEditor"))%>" size="10" readonly="true">
				<input name="realname" class="rdonly" value="<%=trim(rs("realname"))%>" size="10" readonly="true">
				<input name="nickname" class="rdonly" value="<%=trim(rs("nickname"))%>" size="10" readonly="true">
			</TD>
		</TR>
		<TR>
...
<script language="javascript">
	...
	function formBack() {
		window.location.href = "/KnowledgeForum/KnowledgeDiscussList.asp?questionId=<%=questionId%>";
	}
</script>
<%
	end if
