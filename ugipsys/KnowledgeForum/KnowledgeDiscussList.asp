...
<body>
	...
	<div id="FormName">
		...
		<Form id="Form2" name="reg" method="POST" action="KnowledgeDiscussList.asp?questionId=<%=questionId%>">
		<INPUT TYPE=hidden name=submitTask value="">
		<INPUT TYPE="hidden" name="selectedItems" value="">
		<table cellspacing="0" id="ListTable">
		<tr>
			<th class="First" scope="col">&nbsp;</th>
			<th class=eTableLable>討論內容</th>
			<th class=eTableLable>發佈者</th>
			<th class=eTableLable>意見數</th>
			<th class=eTableLable>狀態</th>
			<th class=eTableLable>討論暫存狀況</th>	
		...
		<tr>
			...
			<TD class=eTableContent>
				<font size=2>
					<A href="/KnowledgeForum/KnowledgeDiscussContent.asp?questionId=<%=questionId%>&discussId=<%=rs("iCUItem")%>&nowPage=<%=request.querystring("nowPage")%>&pagesize=<%=request.querystring("pagesize")%>">
					<%
			...
			<TD class=eTableContent><font size=2><%=rs("CommandCount")%></font></td>
...
<script language="javascript">
	...
	function formBack() {
		...
		window.location.href = "/KnowledgeForum/cp_question.asp?iCUItem=<%=questionId%>&nowPage=<%=request.querystring("nowPage")%>&pagesize=<%=request.querystring("pagesize")%>";
		}
	}	
</script>
<%
