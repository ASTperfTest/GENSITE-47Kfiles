...
%>
		<form id="Form1" method="POST" name="reg" action="cp_question_Act.asp?iCUItem=<%=trim(RSreg("iCUItem"))%>" >
			...
			<input type="hidden" name="htx_ictunit">
			<input type="hidden" name="htx_ieditor">
			<input type="hidden" name="htx_deditDate">
			<input type="hidden" name="htx_Created_Date">
			<input type="hidden" name="htx_siteId">
			<input type="hidden" name="htx_ParentIcuitem">
			<table cellspacing="0">
			...
			<TR>
				...
				<TD colspan="6" class="eTableContent">
					<input type="checkbox" name="htx_vgroup" value="A" <% if RSreg("vGroup") = "A" then %> checked <% end if %> >
					<font size=2>疑難知識</font>
				</TD>
			</TR>
			<TR>
			...
			<TR>
				...
				<TD colspan="6" class="eTableContent">
					...
					<INPUT TYPE="hidden" name="status" value="<%=RS4("status")%>">
				</TD>
			</TR>			
			</TABLE>
