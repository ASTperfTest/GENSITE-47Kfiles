...
if request("submitTask") = "delete" then
	...
	'2010.04.29 Add by Ivy 活動新增處理 在include的檔案中有判斷是否為活動時間
	UnusableActivityKnowledge iCUItem
...
if request("submitTask") = "UPDATE" then
	...
	sql2 = "UPDATE KnowledgeForum SET Status = '"& status &"' WHERE gicuitem = "& "'"  & iCUItem & "'"
	conn.execute(sql2)
	'刪除救回end
	...
	conn.execute(sql)
	'2010.04.29 Add by Ivy 活動新增處理 在include的檔案中有判斷是否為活動時間
	if vgroup = "A" And fctupublic = "Y" And xnewWindow = "N" And status = "N" then
		ActivityKnowledge iCUItem
		'response.write "Y"
	
	elseif vgroup="OFF" then
		UnusableActivityKnowledge iCUItem
		'公開所有暫存討論
		PublicTempDiscuss iCUItem
	else
	
		UnusableActivityKnowledge iCUItem
		'response.write "vgroup=" & vgroup & ";" & "fctupublic=" & fctupublic & ";" & "status=" & status & ";" & "xnewWindow=" & xnewWindow & ";"
	end if
...
<% Sub showDoneBox(lMsg, link) %>
  <html>
    ...
    <body>
			<script language=vbs>
			  alert("<%=lMsg%>")	    				
			  document.location.href = "KnowledgeForumlist.asp"
			</script>
    </body>
  </html>
<% End sub %>