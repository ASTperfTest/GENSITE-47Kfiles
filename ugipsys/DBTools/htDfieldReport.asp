...
'	response.end
	...
	set formDom = rptXmlDoc.selectSingleNode("DataSchemaDef/dsTable[tableName='" & formID & "']")
				set nDate = rptXmlDoc.createElement("reportDate")
				nDate.text = date()
				formDom.insertBefore nDate, formDom.selectSingleNode("tableName")

	
	set oxsl = server.createObject("microsoft.XMLDOM")
	  oxsl.async = false
		xv = oxsl.load(server.mappath("fieldReport.xsl"))
'	response.write vo.transformNode(oxsl)
	Response.ContentType = "text/HTML" 
	response.write replace(formDom.transformNode(oxsl),"<META http-equiv=""Content-Type"" content=""text/html; charset=UTF-16"">","")
	response.end
...
cTab = ""
	...
	for each param in paramList
		...
		select case param.selectSingleNode("dataType").text
		  case "varchar"
		  	cSQL = cSQL & cvbCRLF & ctab & paramCode & " varchar(" & paramSize & ")"
		  case "char"
		  	cSQL = cSQL & cvbCRLF & ctab & paramCode & " char(" & paramSize & ")"
		  case else
