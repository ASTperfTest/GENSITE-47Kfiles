...
<%Sub initForm() %>
	...
	reg.htx_dbName.value= "<%=qqRS("dbName")%>"
	reg.htx_dbDesc.value= "<%=qqRS("dbDesc")%>"
	reg.htx_dbIP.value= "<%=qqRS("dbIP")%>"
    end sub
...
'  End if
	...
	IF reg.htx_dbId.value = Empty Then
		...
		exit sub
	END IF
	IF reg.htx_dbName.value = Empty Then
		MsgBox replace(nMsg,"{0}","資料庫名稱"), 64, "Sorry!"
		reg.htx_dbName.focus
		exit sub
	END IF
