...
%> 【<%=RS("actName")%>&nbsp;<%=RS("dtNote")%>】
  ...
  <tr>
    <td class="FormLink" valign="top" align=right>
		<a href="paPsnPickByNum.asp">依亂數選</a>	    
		<a href="paPsnPickByDay.asp">依日期選</a>	    
       <a href="paPsnYesList.asp">正取名單</a>	    
       <a href="paPsnBackList.asp">備取名單</a>	    
...
<%if RS("actname")="童年往事－ 竹童玩製作" or RS("actname")="來跟植物做朋友" then 	
  	response.write RSreg("ckValue")
  else 
	response.write RSreg("myno")
 end if	
%>	
</font></td>
...
<%=RSreg("emergContact")%>
	...
	<TD class=whitetablebg><font size=2 title="<%=RSreg("myorg")%>">
<%=RSreg("myorg")%>
</font></td>
	<TD class=whitetablebg><font size=2 title="<%=RSreg("Addr")%>">
<%=RSreg("Addr")%>
