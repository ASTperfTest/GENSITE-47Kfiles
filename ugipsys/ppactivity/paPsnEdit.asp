...
'  End if
	...
	IF reg.htx_cDate.value = Empty Then 
		MsgBox replace(nMsg,"{0}","建檔日期"), 64, "Sorry!"
		reg.htx_cDate.focus
		exit sub
	END IF
	IF (reg.htx_cDate.value <> "") AND (NOT isDate(reg.htx_cDate.value)) Then
		MsgBox replace(dMsg,"{0}","建檔日期"), 64, "Sorry!"
		reg.htx_cDate.focus
		exit sub
	END IF
	IF blen(reg.htx_myPassword.value) > 12 Then
		MsgBox replace(replace(lMsg,"{0}","登入密碼"),"{1}","12"), 64, "Sorry!"
		reg.htx_myPassword.focus
		exit sub
	END IF
	IF blen(reg.htx_myOrg.value) > 50 Then
		MsgBox replace(replace(lMsg,"{0}","單位"),"{1}","50"), 64, "Sorry!"
		reg.htx_myOrg.focus
		exit sub
	END IF
	IF blen(reg.htx_addr.value) > 50 Then
		MsgBox replace(replace(lMsg,"{0}","位址"),"{1}","50"), 64, "Sorry!"
		reg.htx_addr.focus
		exit sub
	END IF
