...
'------- 記錄 異動 log -------- end --------------------------------------------------------	
	...
	if session("FilmRelated_CorpActor")="Y" then
		conn.execute("delete from FilmCorpInfo where FilmNo="&request.queryString("icuitem"))
		conn.execute("delete from FilmPeopleInfo where FilmNo="&request.queryString("icuitem"))
	end if		
	'----940410 刪除MMO紀錄檔資料
	if checkGIPconfig("MMOFolder") then	
		conn.execute("delete from MMOReferened where MMOID="&request.queryString("icuitem"))
	end if		
...
sub EditInBothCase
	sqlCom = "SELECT htx.*, ghtx.*, xrefNFileName.oldFileName AS fxr_fileDownLoad,rdsdcat,sbaseTableName " 

	sqlFrom = " FROM " & nullText(refModel.selectSingleNode("tableName")) _
		...
		& " Left Join BaseDsd B ON ghtx.ibaseDsd=B.ibaseDsd "

	for each param in refModel.selectNodes("fieldList/field[formList!='' and (inputType='file' or inputType='imgfile')]") 
		xfName = nullText(param.selectSingleNode("fieldName"))
		sqlCom = sqlCom & ",xref" & xfName & ".oldFileName AS fxr_" & xfName
		sqlFrom = sqlFrom & " LEFT JOIN ImageFile AS xref" & xfName _
			& " ON xref" & xfName & ".newFileName=htx." & xfName
	next

	sqlCom = sqlCom & sqlFrom & " WHERE ghtx.icuitem=" & pkStr(request.queryString("icuitem"),"")		
...
'	response.end
    if (request.querystring("S")="Edit" and request.querystring("showType")="") or ((request.querystring("S")="" or request.querystring("S")="Approve" or request.querystring("S")="Ref") and RSreg("showType")="1") then	'----一般資料
	...
	next
  	xshowTypeStr="一般資料式"
  	xshowType="1"
    elseif (request.querystring("S")="Edit" and request.querystring("showType")="2") or ((request.querystring("S")="" or request.querystring("S")="Approve" or request.querystring("S")="Ref") and RSreg("showType")="2") then	'----URL連結
	set allModel2 = session("codeXMLSpec2").documentElement   
	for each param in allModel2.selectNodes("//fieldList/field[showTypeStr='4']") 
		set romoveNode=allModel2.selectSingleNode("field[fieldName='"+param.selectSingleNode("fieldName").text+"']")
		allModel2.removeChild romoveNode
	next
  	xshowTypeStr="URL連結式"
  	xshowType="2"
    elseif (request.querystring("S")="Edit" and request.querystring("showType")="3") or ((request.querystring("S")="" or request.querystring("S")="Approve" or request.querystring("S")="Ref") and RSreg("showType")="3") then	'----檔案下載
	set allModel2 = session("codeXMLSpec2").documentElement   
	for each param in allModel2.selectNodes("//fieldList/field[showTypeStr='4']") 
		set romoveNode=allModel2.selectSingleNode("field[fieldName='"+param.selectSingleNode("fieldName").text+"']")
		allModel2.removeChild romoveNode
	next
  	xshowTypeStr="檔案下載式"
  	xshowType="3"
    elseif ((request.querystring("S")="" or request.querystring("S")="Approve" or request.querystring("S")="Ref") and (RSreg("showType")="4" or RSreg("showType")="5")) then	'----引用資料
    	'----Load XSL樣板
   	set htPageDom = session("codeXMLSpec")
...
<div id="FuncName">
	...
	<div id="Nav">
	       ...
	       <% If Trim(session("CtUnitName")) = "客語認證線上試題卷" Then %>
...
<div id="FormName">
	<% if xshowType <> "5" then %>
	       <%if (HTProgRight and 8)=8 then%>
	       <a href="<%=HTprogPrefix%>Edit.asp?S=Edit&icuitem=<%=request.queryString("icuitem")%>&phase=edit">一般資料式</a>&nbsp;&nbsp;	    
	       <a href="<%=HTprogPrefix%>Edit.asp?S=Edit&icuitem=<%=request.queryString("icuitem")%>&showType=2&phase=edit">URL連結式</a>&nbsp;&nbsp;	    
	       <a href="<%=HTprogPrefix%>Edit.asp?S=Edit&icuitem=<%=request.queryString("icuitem")%>&showType=3&phase=edit">檔案下載式</a>&nbsp;&nbsp;	    
	       <%end if%><br>
       	<%end If %>     	       	
	<%=HTProgCap%>&nbsp;
...
Sub doUpdateDB()
    ...
    if session("FilmRelated_CorpActor")="Y" then
    	...
    	if xUpForm("htx_ProductionCompanyA")<>xProductionCompanyA then FilmRelatedStr=FilmRelated("edit","Corp","製作業",request.queryString("icuitem"),xUpForm("htx_ProductionCompanyA"))
    	if xUpForm("htx_DistributorsA")<>xDistributorsA then FilmRelatedStr=FilmRelated("edit","Corp","發行業",request.queryString("icuitem"),xUpForm("htx_DistributorsA"))
	...
	for each param in allModel.selectNodes("dsTable[tableName='CuDtGeneric']/fieldList/field[formList!='' and identity!='Y']") 
		...
		elseif nullText(param.selectSingleNode("fieldName"))="fctupublic" and session("CheckYN")="Y" then	
