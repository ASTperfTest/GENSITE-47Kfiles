...
<p>網站資料單元點閱統計</p>
<form action="cat_sta.asp" method="post">
...
<%
  ...
  sql="select u.iCuItem, max(n.sTitle) as UnitName,( count(*)*130) as HitNumber from gipHitPage AS u LEFT JOIN CuDTGeneric AS n ON u.iCuItem=n.iCuItem where hitTime between N'" & date1 & "' AND N'" & date2 & "' group by u.iCuItem order by count(*) DESC"
  set rs=conn.Execute(sql)
%>
<table border="1" width="100%">
  <tr>
    <td width="23%">資料單元ID</td>
    <td width="26%">資料單元名稱</td>
    <td width="74%">點閱次數</td>
  </tr>
   <% do while not rs.eof  %> 
  <tr>
