     	...
     	<td class=center height=100 width=60 onclick="editCall '<%=RSreg("iCUItem")%>','<%=RSreg("MMOFolderName")%>'">
     	  ...
     	  <%if not isNull(RSreg("MMOFileName")) then%>
     	    ...
     	    if not isnull(RSreg("MMOFileIcon")) then
     	    	count = count + 1
     	    %>
     	    	<img id="logo_MMOFileName<%=count%>" name="logo_MMOFileName<%=count%>"  src="<%=session("mySiteURL")%>/public/MMO/<%=RSreg("MMOFolderName")%>/<%'=RSreg("MMOFileIcon")%><%=RSreg("MMOFileName")%>">
     	    <%else
     	    	xFileExt="":xIcon=""
...
<!--
var nWidth = 50;
//alert(document.form1.logo_MMOFileName1.width);
function ChangeImageSize()
{
<%
for k=1 to count
%>	
//	alert("ChangeImageSize<%=k%>");
	if(document.form1.logo_MMOFileName<%=k%>.width > nWidth)
	{
	 document.form1.logo_MMOFileName<%=k%>.height = nWidth*document.form1.logo_MMOFileName<%=k%>.height/document.form1.logo_MMOFileName<%=k%>.width;
	 document.form1.logo_MMOFileName<%=k%>.width = nWidth;  
	}	
<%
next
%>
...
for k=1 to count
%>
	document.form1.logo_MMOFileName<%=k%>.onload = ChangeImageSize;
<%
next
%>	
-->
