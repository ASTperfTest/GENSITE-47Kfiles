...
<%Sub showHTMLHead() %>
	...
	<table border="0" width="100%" cellspacing="0" cellpadding="0">
	  <tr>
		...
		<%end if%>		
		<%if (HTProgRight and 8)=8 then%>
			<A href="CTDSDSwap.asp?ctUnit=<%=RSreg("ctUnitId")%>&baseDSD=<%=RSreg("ibaseDsd")%>">變換範本</A>
		<%end if%>		
		<%if (HTProgRight and 4)=4 then%>
			<A href="CtUnitAdd.asp" title="新增">新增</A>
		<%end if%>
