...
<%
response.write request("xn") & "<HR>"
	dim xNewIdentity, xNewName
...
sub clientInitForm	'---- 新增時的表單預設值放在這裡
	reg.htx_CtUnitName.value = "<%=request("xn")%>"
...
'  End if
	...
	IF blen(reg.htx_CtUnitName.value) > 100 Then
		MsgBox replace(replace(lMsg,"{0}","主題單元名稱"),"{1}","30"), 64, "Sorry!"
...
'---- ??d{Xbo	ApU?A?] errMsg="xxx"  exit sub ------
...
'	response.end
	...
	xNewIdentity = RSx(0)
	xNewName = xUpForm("htx_CtUnitName")
...
'	document.location.href = "<%=doneURI%>"
	window.opener.xReturn "<%=xNewIdentity%>"&";;;"&"<%=xNewName%>"
