...
'--xxxEDIT.asp--------------------------
	...
	for each param in refModel.selectNodes("fieldList[tableType='generic']/field[isPrimaryKey='Y']")
		...
		sqlWhere = sqlWhere & "htx." & param.selectSingleNode("fieldName").text _
			& "="" & pkStr(request.queryString(""" & param.selectSingleNode("fieldName").text & """),"""")"
		sqlWhere2 = sqlWhere2 & param.selectSingleNode("fieldName").text _
			& "="" & pkStr(request.queryString(""" & param.selectSingleNode("fieldName").text & """),"""")"
		xcount = xcount + 1
	next
	sqlStr = ct & "SQL = ""DELETE FROM " & GTable & " WHERE " & sqlWhere2
	xfout.writeline sqlStr
	xfout.writeline ct & "conn.Execute(SQL)"
	sqlStr = ct & "SQL = ""DELETE FROM " & DTable & " WHERE " & sqlWhere2
	xfout.writeline sqlStr
	...
	xSelect = "htx.*"
	for each param in refModel.selectNodes("fieldList[tableType='detail']/field[(not (isPrimaryKey)) or (not (isPrimaryKey='Y'))]")
		refField = param.selectSingleNode("fieldName").text
		xSelect = xSelect & ", htxD." & refField
	next
	
	xFrom = "(" & GTable & " AS htx LEFT JOIN " & DTable & " AS htxD ON htx." & GpKey _
		& " = htxD." & GpKey & ") "
	...
	xfout.writeline ct & "Gsql = ""UPDATE " & refModel.selectSingleNode("fieldList[tableType='generic']/tableName").text & " SET """
	xfout.writeline ct & "Dsql = ""UPDATE " & refModel.selectSingleNode("fieldList[tableType='detail']/tableName").text & " SET """

	for each x in htFormDom.selectNodes("//pxHTML//refField")
	    processUpdate x.text
    next
	xfout.writeLine ct & "Gsql = left(Gsql,len(Gsql)-1) & "" WHERE " & sqlWhere2
	xfout.writeline ct & "conn.Execute(Gsql)"
	xfout.writeLine ct & "sql = left(Dsql,len(Dsql)-1) & "" WHERE " & sqlWhere2
...
'	xfout.writeLine ct&"IF request(""htx_" & refField & """) <> """" Then"
	select case paramType
	  case "calc"
	  case "integer"
			xfout.writeLine ct&ct&GD&"sql = " & GD & "sql & """ & refField & "="" & " & "drn(""htx_" & refField & """)"
	  case else
			xfout.writeLine ct&ct&GD&"sql = " & GD & "sql & """ & refField & "="" & " & "pkStr(xUpForm(""htx_" & refField & """),"","")"
	end select
...
'	response.write "<HR>" & xstr & "<HR>"
	...
	xfout.writeLine ct&ct&ct&"Gsql = Gsql & """ & refField & "="" & " & "pkStr(nfname,"","")"
	...
	xfout.writeLine ct&ct&"Gsql = Gsql & """ & refField & "=null,"""
	xfout.writeLine ct&"  end if"
	xfout.writeLine ct&"END IF"
