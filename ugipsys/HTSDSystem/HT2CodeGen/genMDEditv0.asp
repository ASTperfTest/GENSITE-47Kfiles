...
'--xxxEDIT.asp--------------------------
	...
	for each param in refModel.selectNodes("fieldList/field[mpKey='Y']")
		xfout.writeLine ct & "mpKey = mpKey & ""&" & param.selectSingleNode("fieldName").text _
			& "="" & request(""htx_" & param.selectSingleNode("fieldName").text & """)"
	next	
...
sub processUpdate(xstr)
	...
	if nullText(param.selectSingleNode("inputType")) = "readonly" then	exit sub
	paramType = param.selectSingleNode("dataType").text

'	xfout.writeLine ct&"IF request(""htx_" & refField & """) <> """" Then"
  if nullText(param.selectSingleNode("saveDefault/type")) = "" then	
	select case paramType
  	...
  	lhs = ct&ct&"sql = sql & """ & refField & "="" & "
  	select case param.selectSingleNode("saveDefault/type").text
	  case "value"
	  	rhs = cq & param.selectSingleNode("saveDefault/set").text & "," & cq
	  case "sqlFunc"
	  	rhs = cq & param.selectSingleNode("saveDefault/set").text & "," &  cq
	  case "serverFunc"
	  	rhs = "pkstr(" & param.selectSingleNode("saveDefault/set").text & ","","")"
	  case "session"
	  	rhs = "pkstr(session(" & param.selectSingleNode("saveDefault/set").text & "),"","")"
	end select
	...
	if nullText(param.selectSingleNode("inputType")) <> "calc" then
	  select case nullText(param.selectSingleNode("inputType"))
	  	...
	  	case "refCheckboxOther"
			xfout.writeLine ct & "initOtherCheckbox ""htx_" & refField & """," & cq & cl & "=qqRS(""" & refField & """)" & cr & cq _
				& ", ""other_" & refField & cq	
	  	case "refCheckbox"
