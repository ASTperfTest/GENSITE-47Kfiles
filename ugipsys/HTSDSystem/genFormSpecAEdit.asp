...
<%
  ...
  if oxml.parseError.reason <> "" then
	...
	LoadXML = server.mappath("/HTSDSystem/formSpecAEdit0.xml")
  ...
  if nxml.parseError.reason <> "" then
	...
	if lCase(right(progPrefix,3)) = "add" then
		progPrefix = left(progPrefix,len(progPrefix)-3)
	elseif lCase(right(progPrefix,4)) = "edit" then
