...
<%
  ...
  if oxml.parseError.reason <> "" then
	...
	LoadXML = server.mappath("/HTSDSystem/formSpecPsetList0.xml")
  ...
  if nxml.parseError.reason <> "" then
	...
	set setTableNode = nxml.selectSingleNode("//setTable/fieldList")
...
'		response.write ex & "==>" & request.form(ex) & "<BR>"
		if right(ex,12) = "_pickedField" then
			...
			elseif request("detailTable") = xTable then
				setTableNode.selectSingleNode("tableName").text = xTable
				tbPrefix = "dhtx."
...
'				xFieldNode.selectSingleNode("fieldName").text = xFldArray(xfi)
				...
				if request("detailTable") = xTable then
'					response.write "haha<HR>"
					set xsetFieldNode = xFieldNode.cloneNode(true)
					xsetFieldNode.appendChild setTableNode.selectSingleNode("setNew").cloneNode(true)
					setTableNode.appendChild xsetFieldNode
					xparamKind.text = "input"
				end if
...
'				xFieldNode.appendChild xparamKind
			...
			for each xpk in fxml.selectNodes("//dsTable[tableName='" & xTable & "']/fieldList/field[isPrimaryKey='Y']")
				...
				if request("detailTable") = xTable AND _
					nullText(setTableNode.selectSingleNode("field[fieldName='" _
					& xpk.selectSingleNode("fieldName").text & "']")) = "" then
					setTableNode.appendChild xpk.cloneNode(true)
				end if
			next
