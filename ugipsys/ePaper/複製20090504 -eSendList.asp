...
<%
 ...
 'fSql=Request.QueryString("strSql")
	...
	fSql = "SELECT htx.*, m.account, m.realname" _
		...
		& " FROM EpSend AS htx LEFT JOIN Member AS m ON htx.email=m.email" _
		& " WHERE htx.ePubID=" & pkstr(request("ePubID"),"") _
		& " ORDER BY htx.email"
...
<script Language=VBScript>
  dim gpKey
     sub GoPage_OnChange      
           ...
           document.location.href="<%=HTProgPrefix%>List.asp?epubid=<%=epubid%>&nowPage=" & newPage & "&pagesize=<%=PerPageSize%>"    
     end sub      
     
     sub PerPage_OnChange                
           newPerPage=reg.PerPage.value
           document.location.href="<%=HTProgPrefix%>List.asp?epubid=<%=epubid%>&nowPage=<%=nowPage%>&pagesize=" & newPerPage                    
     end sub 
