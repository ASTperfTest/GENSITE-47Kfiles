...
<%Sub initForm() %>
	...
	reg.htx_pubType.item(CInt("<%=qqRS("pubType")%>")).checked= true
    end sub
	...
	<table border="0" width="100%" cellspacing="0" cellpadding="0">
	  <tr>
		...
		<%if (HTProgRight and 8)=8 then%>
                <a href="#" onclick="ePubSendDemo()" title="試發">試發</a>&nbsp;&nbsp;
				<A href="ePubSend.asp?ePubID=<%=request.queryString("ePubID")%>" title="發送">發送</A>
			
			<a href="JavaScript:ePubGen();" title="產生內容檔">產生內容檔</a>
		<%end if%>
...
<%Sub showDoneBox(lMsg) %>
     ...
     <script language=vbs>
       	alert("<%=lMsg%>")
	    document.location.href="<%=HTprogPrefix%>List.asp?epTreeID=<%=xUpForm("epTreeID")%>&<%=Session("QueryPage_No")%>"
     </script>
     </body>
     </html>
<%End sub '---- showDoneBox() ---- %>
<script language=vbs>
sub ePubGen()
     	window.open "ePubGen.asp?epubId=<%=request.queryString("epubId")%>&eptreeid=<%=request.queryString("eptreeid")%>"
end sub
...
<script language=javascript>
	function ePubSendDemo(){
		demoMail = document.getElementById("htx_ePubSendDemoMail");  
		if(demoMail.value.replace(/(^\s*)|(\s*$)/g,"") == ""){
			alert("請輸入試發用mail");
		}else{
			window.open("ePubSendDemo.asp?ePubID=<%=request.queryString("ePubID")%>&ePubMail=" + demoMail.value.replace(/(^\s*)|(\s*$)/g,""))
		}
	}
</script>
