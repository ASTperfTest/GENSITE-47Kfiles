...
'----------HyWeb GIP DB CONNECTION PATCH----------
		...
		While Not nrs.Eof 					
			os = os & GetXmlContent( nrs("ArticleId"), nrs("CtRootId") )			
			nrs.MoveNext			
		Wend
		os = os & "</epSection>"
  ...
  '----Load epaper.xsl
	...
	Function GetXmlContent( aid, rid )
		...
		If rid = "1" Then
			...
			str = str & "<xPostDate>" 					& newrs("xPostDate") 												& "</xPostDate>"      
    ...
    ElseIf rid = "2" Then  
      ...
      sql = ""	
			...
			sql = sql & "CtUnit.CtUnitId, InfoUser.UserName, CuDTGeneric.xPostDate, Dept.deptName, CuDtGeneric.ibaseDsd, CatTreeNode.CtNodeID "
			sql = sql & "FROM CuDTGeneric INNER JOIN CatTreeNode ON CuDTGeneric.iCTUnit = CatTreeNode.CtUnitID INNER JOIN CatTreeRoot ON "
			sql = sql & "CatTreeNode.CtRootID = CatTreeRoot.CtRootID INNER JOIN CtUnit ON "
			sql = sql & "CatTreeNode.CtUnitID = CtUnit.CtUnitID INNER JOIN InfoUser ON "
			sql = sql & "CuDTGeneric.iEditor = InfoUser.UserID INNER JOIN Dept ON CuDTGeneric.iDept = Dept.deptID "
    ...
    ElseIf rid = "4" Then
    	...
    	sql = ""
			...
			str = str & "<xPostDate>" 					& kmrs("ONLINE_DATE") 											& "</xPostDate>"
