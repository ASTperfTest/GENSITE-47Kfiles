...
'		msgBox xs.id & "==>" & xs.style.top
		...
		xSpanArray(xsaCount,5) = xs.style.posWidth
	next
'	msgBox xsaCount
	sortPos 40
end sub
...
'		msgBox xSpanArray(xSortArray(xTopI,1),1)
		...
		if xSpanArray(xSortArray(xTopI,1),3) <> 40 then _
			rxTop = rxTop - xSpanArray(xSortArray(xTopI-1,1),4) - 5
...
'  	fcolor=showModalDialog("editor_color.htm",false,"dialogWidth:106px;dialogHeight:156px;status:0;")
  	fcolor=showModalDialog("ws/fieldXMLspecEdit.asp?fname="&mid(mObj.id,4),false,"dialogWidth:600px;dialogHeight:400px;status:0;")
...
'		mObj.style.borderStyle = "groove"
	...
	elseif (xmid = "") and (mObj.tagName = "SPAN") then 
		...
		dxmid = mObj.id
		mObj.style.borderStyle = "groove"
		mObj.style.borderWidth = 1
		mObj.style.zIndex = 1
'		mObj.style.height = mObj.clientHeight
'		mObj.style.width = mObj.clientWidth
...
'			xObj.style.left = xhDivPos
    ...
    document.all(xmid).style.borderWidth = 0
