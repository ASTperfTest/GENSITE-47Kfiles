...
END FUNCTION
	xKeyword=pkStr(request.querystring("Actor"),"")
...
'----------HyWeb GIP DB CONNECTION PATCH----------
    		...
    		"Values(42,35,'N','0',"&xKeyword&",N'"&date()&"'); select @@IDENTITY as NewID;"
    	    set RSx = conn.Execute(SQLIM)
    	    xNewIdentity = RSx(0)      
    	    '----新增演職人員表
    	    SQLIS="Insert Into ActorInformation(giCuItem) values("&xNewIdentity&")"
    	    conn.execute(SQLIS)	
