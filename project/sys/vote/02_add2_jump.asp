...
%>        
        <tr>
          ...
          <td>
            ...
            <input type="radio" name="type<%=i%>" value="2"<% if m012_type = "2" then %> checked<% end if %>> 複選&nbsp;<br />
            是否提供意見回應區塊： 
            <input type="radio" name="textarea<%=i%>" value="1" <% if m012_textarea = "1" then %> checked <% end if %> /> 提供&nbsp;
            <input type="radio" name="textarea<%=i%>" value="2" <% if m012_textarea = "2" then %> checked <% end if %> /> 不提供
          </td>
        </tr>
				<% end if %>
        <tr>
