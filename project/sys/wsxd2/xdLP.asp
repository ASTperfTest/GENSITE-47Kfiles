...
'目前程式位置在
progPath="D:\hyweb\GENSITE\project\sys\wsxd2\xdLP.asp"

'--------- (可修改)要檢查的 request 變數名稱 --------
genParamsArray=array("ctNode", "ctunit", "mp", "xq_xCat", "xq_xCat2", "xq_bCat", "xq_bDept", "CtNode", "data_base_id", "CtUnit", "BaseDSD", "KMautoID", "category_id", "KMAutoID", "memID", "gstyle", "debug", "pagesize", "nowPage", "htx_vgroup")
...
'		response.write xFrom & "<HR/>"
	...
	lawID = request("xq_xCat")
	if lawID <> "" then
    	sqllawID = "Select iCuItem,sTitle from CuDTGeneric where topCat='"&lawID&"' and ictunit=1988"
    	set RSlawID = conn.execute(sqllawID)
    	if not RSlawID.eof then
        	response.write "<CatListRelated>"
        	while not RSlawID.eof
        		response.write "<row><mCode>" & RSlawID("iCuItem") & "</mCode><mValue><![CDATA[" & RSlawID("sTitle") & "]]></mValue></row>" & vbCRLF
        		RSlawID.moveNext
        	wend
        	response.write "</CatListRelated>"
    	end if
...
'		& " AND iCtUnit = " & pkstr(xCtUnit,"")
	...
	select case  request.querystring("CtUnit") 
	    case "304" 
	        'Bob 2010/1/27
	        '如果是24節氣，只資料只列到今年底為止，依 "起日"排序  
    		    
	        fSql = fSql & " AND year(avEnd) <= year(getdate()) "	    
	        showClientSqlOrderBy = "ORDER BY year(avBegin) desc, avBegin "
	    case "126"
	        'Bob 2010/04/01
	        '隱藏  首頁 > 產銷專欄 > 總幹事專訪 
	        fSql = fSql & " AND ( (iCtUnit='126' and NewtopCat !='01') or  iCtUnit != '126') "	    
	    case "2199"
	        'Derek 2010/10/29
	        '隱藏  農漁生產地圖資料
	        fSql = fSql & " AND ( rss='N') "	   
...
%>
	<qURL><%=qURL%></qURL>
	<xqURL><%=xqURL%></xqURL>
	<xqURLDeleteAnd1><%=trimLastDelimit(qURL,"&amp;")%></xqURLDeleteAnd1>
	<xqURLDeleteAnd2><%=trimLastDelimit(trimLastDelimit(qURL,"&amp;"),"&amp;")%></xqURLDeleteAnd2>
...
If not RSreg.eof then
	...
	if request("ctNode") = "1584" Then
		if request("xq_xCat") <> "" Then xURL = xURL & "&amp;xq_xCat=" & request("xq_xCat")
		if request("htx_vgroup") <> "" then xURL = xURL & "&amp;htx_vgroup=" & request("htx_vgroup")
	end if
...
sc="select distinct realname,money,nickname from (SELECT name,money, (RANK() OVER (PARTITION BY name ORDER BY money DESC )) as rno FROM flashgame  )as a, member as b where a.rno=1 and a.name=b.account order by money desc"
...
Do While Not rsm.EOF
  ...
  if 	rsm("Prize")=0 then
  	response.Write "<Prize>100</Prize>"
  else
  	response.Write "<Prize>" & FormatNumber(2/cint(rsm("Prize"))*100,0) & "</Prize>"
	end if
	response.Write "</PrizeRate>"
rsm.MoveNext
Loop
...
Do While Not rsm.EOF
  ...
  if 	rsm("Prize")=0 then
  	...
  	response.Write "<Prize>" & FormatNumber(5/cint(rsm("Prize"))*100,0) & "</Prize>"
  end if
...
Do While Not rsm.EOF
  ...
  if 	rsm("Prize")=0 then
  	...
  	response.Write "<Prize>" & FormatNumber(25/cint(rsm("Prize"))*100,0) & "</Prize>"
	end if
	response.Write "</PrizeRate>"
rsm.MoveNext
Loop
...
Do While Not rsm.EOF
	response.Write "<PrizeRate>"
	'response.Write "<Prize>" & 50/cint(rsm("Prize"))*100 & "</Prize>"
  if 	rsm("Prize")=0 then
  	response.Write "<Prize>100</Prize>"
  else
	 response.Write "<Prize>" & FormatNumber(50/cint(rsm("Prize"))*100,0)  & "</Prize>"
	end if
	response.Write "</PrizeRate>"
rsm.MoveNext
Loop
...
'// purpose: 去掉字串裡, 某個符號左邊的字元.
'// input:
'//     inputString, 字串
'//     myDelimit, 某個符號
'// ex: ret = getLastDelimit(inputString,Delimit)
function getLastDelimit(byval inputString, byval myDelimit)
    dim returnValue
    dim tempIndex
    dim tempString
    ...
    if inputString <> "" and myDelimit <> "" then
        if instr(inputString,myDelimit) > 0 then
            tempString=StrReverse(inputString)
            myDelimit = StrReverse(myDelimit)
            tempIndex = instr(tempString,myDelimit)
            returnValue = right(inputString,tempIndex-1)
        end if
    end if
    getLastDelimit = returnValue
end function
%>