...
Partial Class knowledge_myknowledge_record2010
  ...
  Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    ...
    End If
    '-----------------------------------------------------------------------------
    TabText.Text = "<ul><li class=""current""><a href=""/knowledge/myknowledge_record.aspx""><span>2010KPI活動積分</span></a></li>" 
  ...
  Function GetMemberPicPath(ByVal memberId As String) As String
    ...
    Dim ConnString As String = "server=127.0.0.1;database=mGIPcoanew3;uid=hyGIP;pwd=hyweb"
    ...
    Try
      ...
      If myReader.Read Then
        If myReader("mCode") = "1" Then
                    sb.Append("<tr><th width=""30%"">等級：[入門級會員]</th><th>本年度積分</th></tr>")
                ElseIf myReader("mCode") = "2" Then
                    sb.Append("<tr><th width=""30%"">等級：[進階級會員]</th><th>本年度積分</th></tr>")
                ElseIf myReader("mCode") = "3" Then
                    sb.Append("<tr><th width=""30%"">等級：[高手級會員]</th><th>本年度積分</th></tr>")
                ElseIf myReader("mCode") = "4" Then
                    sb.Append("<tr><th width=""30%"">等級：[達人級會員]</th><th>本年度積分</th></tr>")
                Else
                    sb.Append("<tr><th width=""30%"">等級</th><th>2010KPI活動積分</th></tr>")
        End If
      End If
      If Not myReader.IsClosed Then
        myReader.Close()
      ...
      myCommand.Dispose()
			...
			'隱藏kpi加權比重以及總分
			sb.Append("<tr><th>網站瀏覽得點 </th><td width='35%'><span class=""red01"">" & thisY_GradeBrowse & "</span> (瀏覽行為原始得點)</td></tr>")
            sb.Append("<tr><th>使用頻率得點 </th><td><span class=""red01"">" & thisY_GradeLogin & "</span> (登入行為原始得點)</td></tr>")
            sb.Append("<tr><th>參與互動得點 </th><td><span class=""red01"">" & thisY_Share & "</span> (互動行為原始得點)</td></tr>")
            sb.Append("<tr><th>內容價值得點 </th><td><span class=""red01"">" & thisY_contentTotal & "</span> (內容價值原始得點)</td></tr>")
            sb.Append("<tr><th>活動加值得點 </th><td><span class=""red01"">" & thisY_contentADDTotal & "</span> (活動參與原始得點)</td></tr>")
            'sb.Append("<tr><th>總得點</th><td><span class=""red01"">" & calculateTotal & "</span> </td><td>" & thisY_Total & "</td></tr>")
            sb.Append("<tr><td colspan='3'>．請各位會員注意!!本表統計至2010/11/30 早上10點止</td></tr>")
        Catch ex As Exception
            Throw ex
        Finally
            If myConnection.State = ConnectionState.Open Then
