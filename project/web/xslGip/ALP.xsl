		...
		<ul>
			<xsl:for-each select="Article">
				<li>
					...
					<xsl:value-of disable-output-escaping="yes" select="ArticleField[fieldName='xpostDate']/Value" />
					<span><xsl:value-of disable-output-escaping="yes" select="substring(ArticleField[fieldName='xbody']/Value,1,50)" /></span>
				</li>
			</xsl:for-each>
		</ul>
	</div>
