...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt"
	...
	<xsl:template match="hpMain">
		<html xmlns="http://www.w3.org/1999/xhtml" lang="zh-TW">
			<head>
				...
				<link rel="stylesheet" type="text/css" href="/css/pedia.css"></link>
				<script src="/js/jquery.js" language="javascript">/**/</script>
				<script src="/js/pedia.js" language="javascript">/**/</script>
				<xsl:if test="//ctNode='1582' or //ctNode='1581' or //ctNode='1583'"><script src="/nocopy.js" language="javascript" ></script></xsl:if>
				<!--Modify by Max 　農業字典-->
				...
				<!--End-->
				<!--Modify by Max 入口網站內單元圖片瀏覽器 2011/06/07-->
					...
					<tr>
						...
						<td class="center">
							...
							<form method="post">
								...
								<!--發佈主題館-->
								<xsl:if test="//SubjectList/Subject">
									<h5>相關主題館</h5>
									<xsl:for-each select="//SubjectList/Subject">
										<a target="_nwGIP">
											<xsl:attribute name="href">
												subject/mp.asp?mp=<xsl:value-of select="subjectmp" />
											</xsl:attribute>
	...
	<xsl:template match="hpMain" mode="textSearch">
		<div class="interact">
			<ul>
				...
				<!-- added by Joey, 2009/09/14, 新增問題反應 -->
				<li class="ask">
					<a href="#" onclick="var domainName=document.domain;window.showModalDialog('http://'+domainName+'/mailbox.asp',self);return false;">系統問題</a>
				</li>
				<input type="hidden" id="type" name="type" value="99"  />
				<input type="hidden"  id="ARTICLE_ID" name="ARTICLE_ID" >
					<xsl:attribute name="value">
						<xsl:value-of select="//queryItems/xItem" />
					</xsl:attribute>
				</input>

				<li class="find">
					<a href="javascript:doSearchReport();">以文找文</a>
				</li>
			</ul>
		</div>
		<script language="JavaScript" type="text/JavaScript">
	...
	<xsl:template match="hpMain" mode="attitudeVote">
		...
		<xsl:if test="//login/status='true'">
			<div class="toScore">
				<h5>這篇文章讓你覺得</h5>
				<form name="AttitudeVoteForm" id="AttitudeVoteForm" action="/AttitudeVoteResult.asp" method="POST">
					...
					<input type="hidden" name="ctNode">
						<xsl:attribute name="value">
							<xsl:value-of select="//queryItems/ctNode" />
						</xsl:attribute>
					</input>
					<input type="hidden" name="mp">
						<xsl:attribute name="value">
							<xsl:value-of select="//queryItems/mp" />
						</xsl:attribute>
					</input>
					<input type="hidden" name="kpi">
						<xsl:attribute name="value">
							<xsl:value-of select="//queryItems/kpi" />
						</xsl:attribute>
					</input>
					<label for="voteResult">
						...
						<img alt="一級棒" title="一級棒">
							<xsl:attribute name="src">
								xslgip/<xsl:value-of select="myStyle"/>/images/exp1.png
							</xsl:attribute>
						</img>
					</label>
					<label for="voteResult">
						...
						<img alt="我喜歡" title="我喜歡">
							<xsl:attribute name="src">
								xslgip/<xsl:value-of select="myStyle"/>/images/exp2.png
							</xsl:attribute>
						</img>
					</label>
					<label for="voteResult">
						...
						<img alt="很實用" title="很實用">
							<xsl:attribute name="src">
								xslgip/<xsl:value-of select="myStyle"/>/images/exp3.png
							</xsl:attribute>
						</img>
					</label>
					<label for="voteResult">
						...
						<img alt="夠新奇" title="夠新奇">
							<xsl:attribute name="src">
								xslgip/<xsl:value-of select="myStyle"/>/images/exp4.png
							</xsl:attribute>
						</img>
					</label>
					<label for="voteResult">
						...
						<img alt="普普啦" title="普普啦">
							<xsl:attribute name="src">
								xslgip/<xsl:value-of select="myStyle"/>/images/exp5.png
							</xsl:attribute>
						</img>
					</label>
					<textarea id="voteDes" name="voteDes" wrap="virtual" onclick="ClearDefault()">我有話要說</textarea>
		...
		<div class="scored">
			...
			<div class="total">
				共有<em>
					<xsl:value-of select="//attributeVote/totalVote" />
				</em>人投摽
			</div>
			<ul>
				<li class="score1" title="一級棒">
					<img>
						<xsl:attribute name="src">
							xslgip/<xsl:value-of select="myStyle"/>/images/null.gif
						</xsl:attribute>
						<xsl:attribute name="alt">
							得票數：<xsl:value-of select="//attributeVote/voteA"/>
						</xsl:attribute>
						<xsl:attribute name="title">
							<xsl:value-of select="//attributeVote/voteA"/>
						</xsl:attribute>
						<xsl:attribute name="height">
							<xsl:value-of select="//attributeVote/voteAPercent"/>
						</xsl:attribute>
					</img>
				</li>
				<li class="score2" title="我喜歡">
					<img>
						...
						<xsl:attribute name="alt">
							得票數：<xsl:value-of select="//attributeVote/voteB"/>
						</xsl:attribute>
						<xsl:attribute name="title">
							<xsl:value-of select="//attributeVote/voteB"/>
						</xsl:attribute>
						<xsl:attribute name="height">
							<xsl:value-of select="//attributeVote/voteBPercent"/>
						</xsl:attribute>
					</img>
				</li>
				<li class="score3" title="很實用">
					<img>
						...
						<xsl:attribute name="alt">
							得票數：<xsl:value-of select="//attributeVote/voteC"/>
						</xsl:attribute>
						<xsl:attribute name="title">
							<xsl:value-of select="//attributeVote/voteC"/>
						</xsl:attribute>
						<xsl:attribute name="height">
							<xsl:value-of select="//attributeVote/voteCPercent"/>
						</xsl:attribute>
					</img>
				</li>
				<li class="score4" title="夠新奇">
					<img>
						...
						<xsl:attribute name="alt">
							得票數：<xsl:value-of select="//attributeVote/voteD"/>
						</xsl:attribute>
						<xsl:attribute name="title">
							<xsl:value-of select="//attributeVote/voteD"/>
						</xsl:attribute>
						<xsl:attribute name="height">
							<xsl:value-of select="//attributeVote/voteDPercent"/>
						</xsl:attribute>
					</img>
				</li>
				<li class="score5" title="普普啦">
					<img>
						...
						<xsl:attribute name="alt">
							得票數：<xsl:value-of select="//attributeVote/voteE"/>
						</xsl:attribute>
						<xsl:attribute name="title">
							<xsl:value-of select="//attributeVote/voteE"/>
						</xsl:attribute>
						<xsl:attribute name="height">
							<xsl:value-of select="//attributeVote/voteEPercent"/>
						</xsl:attribute>
					</img>
				</li>
			</ul>
