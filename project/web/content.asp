...
'目前程式位置在
progPath="D:\hyweb\GENSITE\project\web\content.asp"

'--------- (可修改)要檢查的 request 變數名稱 --------
genParamsArray=array("CuItem", "CtNode")
...
call CheckURL(Request.QueryString)
	...
	'[WctNode
	if request("CuItem") <> "" and request("CtNode") = "" then
		SQLC = "Select ctNodeId from CuDtGeneric CDT Left Join CatTreeNode CN ON CDT.ictunit = CN.ctUnitId where CDT.icuitem='" & replace(request("CuItem"),"'","''") & "'"
		set RSC = conn.execute(SQLC)
		if not RSC.eof then contenturl="xItem="&request("CuItem")&"&ctNode="&RSC(0)&"&mp="&mp
	end if

	'xv = oxml.load(session("myXDURL") & "/wsxd2/xdcp2.asp?" & request.queryString)
	xv = oxml.load(session("myXDURL") & "/wsxd2/xdcp2.asp?" & contenturl)
	'response.write session("myXDURL") & "/wsxd2/xdcp2.asp?" & contenturl
	'response.end
  if oxml.parseError.reason <> "" then 
	...
	oxsl.load(server.mappath("xslGip/" & session("myStyle") & "/cp.xsl"))
'response.write server.mappath("xslGip/" & session("myStyle") & "/cp.xsl")
'response.end
