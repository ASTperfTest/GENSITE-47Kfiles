...
%>
			<item iCuItem="<%=stripHTML( RS("iCuItem") )%>" newWindow="<%=stripHTML( RS("xNewWindow") )%>">
				<title><%=stripHTML( RS("sTitle") )%></title>
				<link><%=deAmp(xURL)%></link>
				<guid isPermaLink="false"><%=stripHTML( RS("iCuItem") )%></guid>
 				<pubDate><%=xPostDateStr%></pubDate>
				<description><![CDATA[<%=stripHTML( RS("xBody") )%>]]></description>
				<% if not isNull( stripHTML( RS("xImgFile") ) ) then %>
					<xImgFile>public/Data/<%=stripHTML( RS("xImgFile") )%></xImgFile>
				<% end if %>
			</item>
<%		
			RS.moveNext
