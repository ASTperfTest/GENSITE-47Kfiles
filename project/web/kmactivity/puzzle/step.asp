...
<%
	...
	If Not RS.Eof Then 
		...
		If cstr(RS("difficult"))<>cstr(diff) Then checkgame = checkgame + 1
		If checkgame<=0 Then '送進來的資料跟資料庫的資料一樣時，顯示錯誤
			Response.write "資料不同步，請按F5重新整理！"
			Response.End()
		End If
	Else
	...
	'扣掉點數
	If Not RS.Eof Then 
		SQL = "update ACCOUNT set Energy=Energy-1  Where Login_ID = '"& loginid &"' "	
		set rs1 = conn.execute (SQL)
	...
	if check="Y" Then 				
		...
		'先檢查是否今天己經玩超過圖庫總數
		SQL = "Select count(*) As Total From GAMEHistory Where Login_ID = '"& loginid &"' And (CONVERT(char(8),gametime,11)=CONVERT(char(8),GETDATE(),11)) And (picState='Y' Or picState='F') "			
		set rs1 = conn.execute (SQL)
		If Not RS1.eof Then 
			...
			Elseif cint(RS1("Total"))=5 Then
				Response.write "f" '回給flash拼圖完成，並且完成5幅拼圖	
				Response.End()	
			Else
