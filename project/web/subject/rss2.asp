...
'目前程式位置在
progPath="D:\hyweb\GENSITE\project\web\subject\rss2.asp"

'--------- (可修改)要檢查的 request 變數名稱 --------
genParamsArray=array("xitem", "ctNode", "count", "xItem", "mp")
...
on error resume next
	...
	Conn.Open session("ODBCDSN")
 	...
 	xTreeNode = 1630
	xIcuitem = request("xitem")
	if pkStrWithSriptHTML(request("ctNode"), "") <> "null"	then	xTreeNode = pkStrWithSriptHTML(request("ctNode"), "")
	
if isnumeric(request("ctNode"))  Then
if CInt(request("ctNode")) > 0 then
  ...
  if not RS.eof then
	...
	sql_t = "SELECT sTitle FROM CuDTGeneric WHERE iCUItem = '"& request("xItem") &"'"
	set rs_t = conn.execute(sql_t)
	title = rs_t("sTitle")
	sql = "SELECT CuDTGeneric.* FROM CuDTGeneric INNER JOIN SubjectForum ON CuDTGeneric.iCUItem = SubjectForum.gicuitem INNER JOIN Member ON CuDTGeneric.iEditor = Member.account "
	sql = sql &" WHERE (SubjectForum.ParentIcuitem = '"& request("xItem") &"') and CuDTGeneric.iCTUnit = '2752' "
	sql = sql &" order by CuDTGeneric.xPostDate DESC"
		
	set RS = conn.execute(sql)
	while not RS.eof
		xURL = session("myWWWSite") & "/ct.asp?xItem="& request("xItem") &"&ctNode="& request("ctNode") &"&mp="& request("mp")
