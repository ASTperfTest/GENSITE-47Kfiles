...
'目前程式位置在
progPath="D:\hyweb\GENSITE\project\web\subject\sitemap.asp"
...
'	response.write session("myStyle") & "hh<HR>"
'	response.end
Response.Expires = -1%>
...
call CheckPoint_BeforeLoadXML(request("mp"), request("ctNode"), request("xItem"))	
	...
	xv = oxml.load(session("myXDURL") & "/wsxd/xdsitemap.asp?mp="&mp)
	'Response.write (session("myXDURL") & "/wsxd/xdsitemap.asp?mp="&mp)
  if oxml.parseError.reason <> "" then 
	...
	'oxsl.load(server.mappath("xslGip/" & session("myStyle") & "/sitemap.xsl"))
