	...
	$.supersized = function( options ) {
		...
		$(window).load(function(){
			...
			if (options.slideshow && options.slides.length > 1){
	    		...
	    		slideshow_interval = setInterval(nextslide, mySlide_interval);	//Initiate slide interval
				...
				if (options.thumbnail_navigation){
					...
					$('#nextthumb').click(function() {
					    ...
					    if(!(isPaused)) slideshow_interval = setInterval(nextslide, mySlide_interval);	//If not paused, resume slideshow
				...
				if (options.navigation){
				    ...
				    $(pauseplay).click(function() {
						...
						if (isPaused){
							...
							//Resume slideshow
							isPaused = false;
				        	slideshow_interval = setInterval(nextslide, mySlide_interval);
