...
'目前程式位置在
progPath="D:\hyweb\GENSITE\project\web\subject\question_login.asp"
...
<form method="post" action="/question_loginact.asp?redirecturl=/knowledge/knowledge_question.aspx?subjectmp=<%=request("mp")%>">
<input type="hidden" name="O_URL" value="<%=Request.ServerVariables("HTTP_REFERER")%>">
<label>帳號：<input name="account2" type="text" size="20" class="txt" /></label>
<label>密碼：<input name="passwd2" type="password" size="20" class="txt" /></label>
...
<%
		...
		sql_mem1 ="SELECT passwd FROM Member where account ='"& Session("memID") &"'"
		set rs_mem1 = Conn.execute(sql_mem1)
%>
		<script language="javascript">
			location.href = "/question_loginact.asp?account2=<%=Session("memID")%>&passwd2=<%=rs_mem1("passwd")%>&redirecturl=/knowledge/knowledge_question.aspx?subjectmp=<%=request("mp")%>"
		</script> 
<%
	end if
%>