...
<!-- BlockA End -->
	...
	<xsl:template match="//BlockA/Article" mode="Style1">
		<xsl:if test="../IsPic = 'Y'">
			<div class="block">		
	...
	<xsl:template match="//BlockA/Article" mode="Style2">
		<div class="BlockA_Div">
			<xsl:if test="../IsPic = 'Y'">		
				<xsl:if test="xImgFile">
					<img class="listimg">
						<xsl:attribute name="src">
	...
	<xsl:template match="//BlockA/Article" mode="Style3">
		 ...
		 <xsl:if test="../IsPic = 'Y'">		
          ...
          <xsl:value-of select="Caption" />
			...
			<!-- 文章內容 end-->		
				......</xsl:if>
				&lt;<a><xsl:attribute name="href">ct.asp?xItem=<xsl:value-of select="@iCuItem" />&amp;ctNode=<xsl:value-of select="../@xNode" />&amp;mp=<xsl:value-of select="//hpMain/mp"/></xsl:attribute>
        		詳全文</a>&gt;
			</p>		
	</xsl:template>
