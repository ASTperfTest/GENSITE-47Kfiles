...
<xsl:template match="hpMain" mode="Sitemap">
	...
	<xsl:for-each select="Sitemap/Menucat">
	 ...
	 <xsl:variable name="thisPosition"><xsl:value-of select="position()" /></xsl:variable>
          <li>
          <xsl:choose>	
          <xsl:when test="xURL!=''">
                <a>
		  ...
		  <xsl:value-of select="$thisPosition" />.<xsl:value-of select="Caption" />     	        		
    	        </a>	
	  </xsl:when>
	  <xsl:otherwise>
			  <a>
			  <xsl:attribute name="href">np.asp?ctNode=<xsl:value-of select="@iCuItem" />&amp;mp=<xsl:value-of select="//mp" /></xsl:attribute>
			  <xsl:attribute name="title"><xsl:value-of select="Caption" /></xsl:attribute> 
	          <xsl:value-of select="$thisPosition" />.<xsl:value-of select="Caption" />
			  </a>
	  </xsl:otherwise>
          ...
          <xsl:if test="@isUnit='C'">
          	<xsl:for-each select="ancestor::hpMain//Sitemap/Menucat[@dataparent=$thisCaptionID]">
				...
				<xsl:variable name="thisPosition2"><xsl:value-of select="position()" /></xsl:variable>
             	<ul> 
            	<li>
            	<xsl:choose>	
          <xsl:when test="xURL!=''">
		  ...
		  <xsl:attribute name="href"><xsl:value-of disable-output-escaping="yes" select="xURL" /></xsl:attribute>
		  <xsl:attribute name="title"><xsl:value-of select="Caption" /></xsl:attribute> 
		  <xsl:value-of select="$thisPosition" />-<xsl:value-of select="$thisPosition2" />.<xsl:value-of select="Caption" />     	        		
    	        </a>	
	  </xsl:when>
	  <xsl:otherwise>
