		...
		function getTextAreaSelection(){
          ...
          elementObj[i].focus();             
			...
			return alertStr ;
                ...
                <xsl:if test="PersonTag/memid !=''">
                <div >
                  <p></p>
                  在本篇文章加入個人標籤：<br/>
                  <input id="tags_22" name="tags3" class="tags" size="80" style="margin-bottom:10px;" readonly="readonly">
                    <xsl:attribute name="value">
                      <xsl:value-of select="PersonTag/Tags"/>
                    </xsl:attribute>
                  </input>
                  <input id="memid" type="hidden">
                    <xsl:attribute name="value">
                      <xsl:value-of select="PersonTag/memid"/>
                    </xsl:attribute>
                  </input>
                  <input id="ArticleId" type="hidden">
                    <xsl:attribute name="value">
                      <xsl:value-of select="PersonTag/ArticleId"/>
                    </xsl:attribute>
                  </input>
                  <input id="tagsave" value="儲存標籤" type="reset" />
                </div>
                </xsl:if>
                <div id="msg"></div>								<div style="text-align: right">
									<a href="#top">
