...
<xsl:template match="hpMain">
	<html xmlns="http://www.w3.org/1999/xhtml">
		...
		<link rel="stylesheet" type="text/css" >
			<xsl:attribute name="href">/xslgip/<xsl:value-of select="//myStyle"/>/css/sitemap.css</xsl:attribute>
		</link>			
		<body>
		<!-- 網站標題／開始 -->
		...
		<table class="Layout" summary="排版表格">
		  <tr>
				...
				<td class="Center">
					...
					<!-- 單元名稱／開始 -->
					<div class="PageTitle"><xsl:value-of select="SitemapTop/Caption"/></div>
					<!-- 單元名稱／結束 -->
		
					<!-- 導覽列／開始 -->
						<xsl:apply-templates select="." mode="SitemapxPath"/>
					<!-- 導覽列／結束 -->
		
					<!-- 網站導覽／開始 -->
						<div class="Sitemap">
							<xsl:apply-templates select="." mode="SitemapTop"/>
