	...
	<html xmlns="http://www.w3.org/1999/xhtml">
		<head>
			...
			<link rel="stylesheet" type="text/css" >
				<xsl:attribute name="href">/xslgip/<xsl:value-of select="//myStyle"/>/css/lp.css</xsl:attribute>
			</link>		
		</head>		
		<body>
		<!-- 網站標題／開始 -->
	...
	<ol class="oList">
		<xsl:for-each select="Article">
			<li>
				...
				<xsl:if test="xURL =''">
					<span class="Title"><xsl:value-of select="Caption" /></span>
				</xsl:if>
				<span class="Date"><xsl:value-of select="PostDate" /></span>
				<span class="Brief"><xsl:value-of select="Abstract" /></span>
			</li>
		</xsl:for-each>	
	</ol>	
...
<xsl:template match="hpMain" mode="pageissue">
	...
	<!--xsl:if test="totRec>$PerPageSize"-->
		<div class="Page"> 
			共 <span class="Number"><xsl:value-of select="totPage" /></span> 頁， <span class="Number"><xsl:value-of select="totRec" /></span> 筆資料
			<xsl:if test="nowPage > 1">
				<a>
					...
					</xsl:attribute><img alt="上一頁"><xsl:attribute name="src">/xslgip/<xsl:value-of select="//myStyle"/>/images/pager_prev.gif</xsl:attribute></img>
				</a>
			</xsl:if>			
			 &amp;nbsp;到第
			...
			<xsl:if test="number(nowPage) &lt; number(totPage)">
				<a>
					<xsl:attribute name="href">
						lp.asp?<xsl:value-of select="qURL" />&amp;nowPage=<xsl:value-of select="nowPage + 1" />&amp;pagesize=<xsl:value-of select="PerPageSize" />
					</xsl:attribute><img alt="下一頁"><xsl:attribute name="src">/xslgip/<xsl:value-of select="//myStyle"/>/images/pager_next.gif</xsl:attribute></img>
				</a>
			</xsl:if>
			&amp;nbsp; 每頁顯示
