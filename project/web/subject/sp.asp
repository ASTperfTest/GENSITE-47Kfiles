...
'目前程式位置在
progPath="D:\hyweb\GENSITE\project\web\subject\sp.asp"

'--------- (可修改)要檢查的 request 變數名稱 --------
genParamsArray=array("ttxsl")
...
'	response.write session("myXDURL") & "/wsxd/xdSP.asp?mp=" & session("mpTree") & "&" & qStr
'	response.end
...
call CheckPoint_BeforeLoadXML(request("mp"), request("ctNode"), request("xItem"))		
	...
	xv = oxml.load(session("myXDURL") & "/wsxd/xdSP.asp?" & qStr & "&memID=" & memID & "&gstyle=" & gstyle)
