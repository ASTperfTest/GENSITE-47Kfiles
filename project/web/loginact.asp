...
'目前程式位置在
progPath="D:\hyweb\GENSITE\project\web\loginact.asp"
...
function uncodDecodeChar(byval inputChar, byval publicKey)
	...
	mp = getMPvalue() 
	publicKey = 18
	'account2 = stripHTML(unicodeDecode(request("account2"),publicKey))
	'passwd2 = stripHTML(unicodeDecode(request("passwd2"),publicKey))
...
<%
	...
	'判斷出生日期是否有效
	validBirthday = nullText(oxml.selectSingleNode("//login/validBirthday"))
	If validBirthday = "True" then 
	'執行原動作
		...
		'----作為傳給主題館參數用
		subjecturl= session("myWWWSite") & "/loginact.asp"
		param = "account=" & account2 & "&passwd=" & passwd2
...
%>
   	<script language="javascript">
			$.post("<%=subjecturl %>","<%=param %>","");
     		alert("登入成功!!");
     		window.location.href = "AspSession.asp?redirecturl=<%=request.querystring("redirecturl")%>";
   	</script>   
<%
...
%>
   	<script language="javascript">
     		alert("會員資料不全，請補充會員資料。");
			window.location.href = "sp.asp?xdURL=Coamember/member_Modify.asp&mp=1&updateMemID=<%=updateMemID%>";
   	</script>   
<%
	End If
