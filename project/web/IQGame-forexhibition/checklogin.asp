...
'目前程式位置在
progPath="D:\hyweb\GENSITE\project\web\IQGame-forexhibition\checklogin.asp"

'--------- (可修改)要檢查的 request 變數名稱 --------
genParamsArray=array("userIdno")
...
%><%
  ...
  if not gamedata.eof then
	   ...
	   'SQL = "select * from MemberFlashLoginLog where account='" & request("userIdno") & "' and LoginDatetime >= '" & Today & "' and LoginDatetime < DATEADD(day, 1, '" & Today & "')"
        ...
        SQL = "select top 1 * from flashGame,CuDTGeneric where CuDTGeneric.iCuItem=flashGame.giCuItem and name = '" & request("userIdno") & "' order by money desc" 
